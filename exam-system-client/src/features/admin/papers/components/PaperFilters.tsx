...
export function PaperFilters({values, onChange, onAdd, onRefresh}: PaperFiltersProps) {
	return <div className="flex flex-wrap items-end gap-3">
		<div className="space-y-1.5">
			<Label className="text-xs text-muted-foreground">试卷名称</Label>
			<Input
					placeholder="搜索名称..."
					value={values.name ?? ''}
		...
		<div className="space-y-1.5">
			...
			<Select
					...
					onValueChange={(val) =>
							onChange({...values, status: val === 'all' ? undefined : val})
					}
			>
