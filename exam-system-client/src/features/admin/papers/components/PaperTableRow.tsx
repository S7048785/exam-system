...
import {PAPER_STATUS, PAPER_STATUS_MAP} from "#/features/admin/papers/utils.ts";
...
export default function PaperTableRow({item, onStatusChange, onEdit, onDelete}: Props) {
	const statusInfo = PAPER_STATUS_MAP[item.status as keyof typeof PAPER_STATUS_MAP]
			...
			<div className="flex items-center gap-1">
				{
						currentAction && <Button
								variant="ghost"
								size="sm"
								onClick={() => onStatusChange({id: item.id, status: currentAction.next})}
						>
							{currentAction.label}
						</Button>
				}
