...
// 题型映射
export const TYPE_MAP: Record<string, { label: string; variant: 'default' | 'secondary' | 'outline' }> = {
...
// 难度映射
export const DIFFICULTY_MAP: Record<string, { label: string; className: string }> = {
...
export function renderChoicesAndAnswer(
    ...
    return (
      <div className="text-sm">
        {choices.map((choice, index) => {
          ...
          const isCorrect = answerSet.has(label) || answerSet.has(choice.content)
          return (
            <div key={index} className={isCorrect ? 'text-green-600 font-medium' : ''}>
  ...
  if (type === 'JUDGE') {
    const isCorrect = answer === 'true' || answer === '正确' || answer === 'T' || answer === 'Y'
    return (
      <span className={`text-sm ${isCorrect ? 'text-green-600 font-medium' : 'text-red-600 font-medium'}`}>
        {isCorrect ? '正确' : '错误'}
      </span>
    )
  }
