...
export default function AiGenerateDialog({
	...
	const [categoryIdForImport, setCategoryIdForImport] = useState<number | null>(null)

	// 生成结果
	const [generatedQuestions, setGeneratedQuestions] = useState<QuestionGenerateDto[]>([])
	...
	const importMutation = useMutation({
		mutationFn: async (data: QuestionGenerateDto[]) => {
			const importInputs: QuestionImportInput[] = data.map((item) => ({
				...
				choices: item.type === 'CHOICE' ? item.choices!.map((choice, index) => ({
					content: choice,
					correct: item.answer.includes(String.fromCharCode('A'.charCodeAt(0) + index)),
					sort: index
				})) : undefined,
				answer: item.answer,
			}))
	...
	// 开始生成
	const handleStartGenerate = useCallback((values: {
		count: number
		type: string
		difficulty: string
	...
	// 关键：动画完全结束后再重置状态
	const handleContentAnimationEnd = (e: React.AnimationEvent | React.TransitionEvent) => {
			...
			<Dialog open={open} onOpenChange={onOpenChange}>
				...
				>
					...
					<div className="flex-1 overflow-auto px-6 py-4">
						...
						{step === 4 && (
								<Step4Success count={generatedQuestions.length} onClose={onOpenChange}/>
						)}
					</div>
				</DialogContent>
			</Dialog>
