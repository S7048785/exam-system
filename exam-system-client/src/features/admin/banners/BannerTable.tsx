...
import {Add01Icon, ReflexIcon} from "@hugeicons/core-free-icons";
...
export default function BannerTable({data, refetch, addBanner, deleteBanner, updateBanner}: BannerTableProps) {
	const [selectedBanner, setSelectedBanner] = useState<BannersDto['BannerController/BANNER_ITEM_ADMIN'] | null>(null);

	const [drawerOpen, setDrawerOpen] = useState<boolean>(false);

	const handleOpenDrawer = (banner?: BannersDto['BannerController/BANNER_ITEM_ADMIN']) => {
		// 这种写法更符合常见的业务场景
	...
	const renderBannerRow = (banner: BannersDto['BannerController/BANNER_ITEM_ADMIN']) => {
		return (
				<TableRow key={banner.id} className="hover:bg-muted/50">
					<TableCell className="h-16 px-4 font-medium">{banner.id}</TableCell>
					<TableCell className="h-16 px-4">
							{banner.imageUrl ? <img src={banner.imageUrl} alt="" className="w-32 object-cover rounded-md"/> :
									<ImageIcon className="size-8 opacity-60 mx-auto"/>}
					</TableCell>
					<TableCell className="h-16 px-4 text-sm text-muted-foreground">
						<Button variant="link" onClick={() => handleOpenDrawer(banner)}>{banner.title}</Button>
					</TableCell>
					<TableCell className="h-16 px-4 text-sm text-muted-foreground">
							...
							<PopoverContent className="w-64">
								<div className="flex flex-col gap-4">
									...
									<div className="flex justify-end gap-2">
										...
										<Button size="sm" variant="destructive" data-id={banner.id} onClick={handleDelete}>确定</Button>
									</div>
								</div>
							</PopoverContent>
						</Popover>
			...
			<>
				<div className={"flex justify-end mb-2"}>
					...
					<Button variant="outline" size="sm" onClick={onRefetchClick}>
						<HugeiconsIcon icon={ReflexIcon} strokeWidth={2}/>
