...
class UserController(
    ...
    fun sendCaptcha(@RequestParam email: String): R<String> {
    ...
    suspend fun loginUser(@RequestBody user: UserLoginInput, response: HttpServletResponse): R<String?> {
        ...
        val token = userService.login(user.email, decryptedPassword)
    ...
    fun registerUser(@RequestBody user: UserRegisterInput): R<String?> {
        ...
        if (!emailService.verifyCaptcha(user.email, user.captcha)) {
            throw ResponseStatusException(HttpStatus.UNPROCESSABLE_ENTITY, "验证码错误或已过期")
        }
