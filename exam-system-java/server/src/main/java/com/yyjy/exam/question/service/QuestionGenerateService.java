...
public class QuestionGenerateService {
    ...
    private String generateExtraRequirements(String type) {
        return switch (type) {
            case "CHOICE" -> PromptConstant.QUESTION_CHOICE_GENERATE_USER_PROMPT;
            case "JUDGE" -> PromptConstant.QUESTION_JUDGE_GENERATE_USER_PROMPT;
            case "TEXT" -> PromptConstant.QUESTION_TEXT_GENERATE_USER_PROMPT;
            default -> throw new BusinessException("题目类型错误");
        };
    }
